...
def change_bootstrap():
  if not base.is_exist("./depot_tools/bootstrap/manifest.txt.bak") and base.is_exist("./depot_tools/bootstrap/manifest.txt"):
...
def fix_ubuntu24():
  ...
  old_cur = os.getcwd()
  target_dir = "third_party/llvm-build/Release+Asserts/lib"
  if base.is_dir(target_dir):
    os.chdir(target_dir)
    if base.is_file("libstdc++.so.6"):
